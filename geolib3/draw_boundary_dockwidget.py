...
class DrawBoundaryDockWidget(QDockWidget, Ui_DrawBoundaryDockWidget):
    ...
    def addCboLayerItem(self):
        ...
        self.ui.cboDemLayer.addItem(self.tr('-- Select DEM Layer--'))
        for layer in layers:
            if isinstance(layer, QgsRasterLayer):
                self.ui.cboDemLayer.addItem(layer.name())
        #Target Layer Combobox
        self.ui.cboTargerLayer.clear()
        self.ui.cboTargerLayer.addItem(self.tr('-- Select strdip Layer--'))
        for layer in layers:
...
class QgsMapToolClick(QgsMapTool):
    ...
    def canvasPressEvent(self, mouseEvent):
        if self.point_count < 3:
            ...
            if elevation == '':
                QMessageBox.information(None, "Information:", self.tr("Can't get elevation data.Please select valid DEM."))
            else:
                #Add text to the row
